...
settings = {}
unless ARGV.size == 5
  puts "Usage: kingdom-splitter sequences.fasta clean.csv contaminated.csv clean_output.fasta contaminated_output.fasta"
  exit
end
...
sequences.each do |entry|
  current = QUERY_SEQ_REGEXP.match(entry.definition)[1]
